...
// Ip header
static void create_ip_header(struct iphdr *iph, const char *source_ip, unsigned short packet_id, const char *dest_ip, int tcp_payload_size) {
    iph->ihl = 5;
    iph->version = 4;
    iph->tos = 0;
...
// Tcp header
static void create_tcp_header(struct tcphdr *tcph, int source_port, int dest_port, int flags, int data_len) {
